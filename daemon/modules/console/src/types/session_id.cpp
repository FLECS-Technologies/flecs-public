...
// limitations under the License.#ifndef SESSION_ID_H

#include "daemon/modules/console/types/session_id.h"
...
session_id_t::session_id_t(std::string id, std::time_t timestamp)
    : _id(std::move(id)), _timestamp(timestamp)
{}
...
auto from_json(const json_t& j, session_id_t& session_id) //
    -> void
{
...
auto to_json(json_t& j, const session_id_t& session_id) //
    -> void
{
    j = json_t({
        {"id", session_id.id()},
