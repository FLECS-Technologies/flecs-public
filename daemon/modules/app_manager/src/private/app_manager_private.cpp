    ...
    -> void
{
    ...
    {
        auto docker_process = process_t{};
        docker_process.spawnp("docker", "network", "ls", "--filter", "name=flecs-macvlan", "--format", "{{.Name}}");
        docker_process.wait(false, true);
        if (docker_process.exit_code() != 0)
        {
            return;
        }
        networks = docker_process.stdout();
    }

    // disconnect all instances from networks
    for (const auto& network : split(networks, '\n'))
    {
        for (const auto& instance : _app_db.all_instances())
        {
            const auto it = std::find(instance.networks.cbegin(), instance.networks.cend(), network);
            if (it != instance.networks.cend())
            {
                auto docker_process = process_t{};
                docker_process.spawnp("docker", "network", "disconnect", network, "flecs-" + instance.id);
                docker_process.wait(false, true);
                if (docker_process.exit_code() != 0)
                {
                    return;
        ...
        auto cidr_subnet = std::string{};
        {
            auto docker_process = process_t{};
            docker_process
                .spawnp("docker", "network", "inspect", "-f", "'{{range .IPAM.Config}}{{.Subnet}}{{end}}'", network);
            docker_process.wait(false, true);
            if (docker_process.exit_code() != 0)
            {
                return;
            }
            cidr_subnet = docker_process.stdout();
        }

        // get gateway configuration
        auto gateway = std::string{};
        {
            auto docker_process = process_t{};
            docker_process
                .spawnp("docker", "network", "inspect", "-f", "'{{range .IPAM.Config}}{{.Gateway}}{{end}}'", network);
            docker_process.wait(false, true);
            if (docker_process.exit_code() != 0)
            {
                return;
            }
            gateway = docker_process.stdout();
        }

        // get adapter configuration
        const auto parts = split(network, '-');
        if (parts.size() < 3)
        {
            return;
        }
        ...
        cidr_subnet.erase(
            std::remove_if(cidr_subnet.begin(), cidr_subnet.end(), [](char c) { return c == '\'' || c == '\n'; }),
            cidr_subnet.end());
        gateway.erase(
            std::remove_if(gateway.begin(), gateway.end(), [](char c) { return c == '\'' || c == '\n'; }),
            gateway.end());

        const auto new_network = "flecs-ipvlan-" + adapter;
        // remove flecs-macvlan network
        {
            auto docker_process = process_t{};
            docker_process.spawnp("docker", "network", "rm", network);
            docker_process.wait(false, true);
            if (docker_process.exit_code() != 0)
            {
                return;
        ...
        // re-create networks as ipvlan
        {
            ...
            docker_process.spawnp(
                ...
                gateway,
                "-o",
                std::string{"parent="} + adapter,
                new_network);
        }

        // reconnect instances
        for (auto& instance : _app_db.all_instances())
        {
            const auto it = std::find(instance.networks.cbegin(), instance.networks.cend(), network);
            if (it != instance.networks.cend())
            {
                const auto pos = it - instance.networks.cbegin();
                auto docker_process = process_t{};
                docker_process.spawnp(
                    "docker",
                    "network",
                ...
                if (docker_process.exit_code() != 0)
                {
                    instance.networks.erase(it);
                    instance.ips.erase(instance.ips.begin() + pos);
                    return;
                }
                instance.networks[pos] = new_network;
                _app_db.insert_instance(instance);
            }
        }
    }
