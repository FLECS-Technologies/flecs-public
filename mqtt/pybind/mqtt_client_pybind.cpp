...
{
    using receive_callback_set_t =
        int (FLECS::mqtt_client_t::*)(FLECS::mqtt_client_t::receive_cbk_t);
    py::class_<FLECS::mqtt_client_t>(m, "mqtt_client")
        .def(py::init())
        .def("connect", (int(FLECS::mqtt_client_t::*)())(&FLECS::mqtt_client_t::connect))
        .def(
            "connect",
            (int(FLECS::mqtt_client_t::*)(const char*, int, int))(&FLECS::mqtt_client_t::connect))
        ...
        .def(
            ...
            (int(FLECS::mqtt_client_t::*)(const char*, int*, int, const void*, int, bool)
                 const)(&FLECS::mqtt_client_t::publish))
        .def(
            "receive_callback_set",
            (receive_callback_set_t)(&FLECS::mqtt_client_t::receive_callback_set))
