...
static int select_app_callback(void* data, int argc, char** argv, char* col_name[])
{
    auto entry = reinterpret_cast<apps_table_entry_t*>(data);
...
int app_db_t::delete_app(const apps_table_primary_t& primary)
{
    ...
    const auto len2 = snprintf(nullptr, 0, apps_table_primary_where_format, primary.app.c_str(), primary.version.c_str());

    auto delete_str = std::make_unique<char[]>(len1 + len2 + 1);

    std::snprintf(delete_str.get(), len1 + 1, delete_statement, apps_table_name);
    std::snprintf(delete_str.get() + len1, len2 + 1, apps_table_primary_where_format, primary.app.c_str(), primary.version.c_str());
...
{
    auto entry = apps_table_entry_t {};

    std::array<const char*, 2> filter = { "app", "version" };
    select_all(apps_table_name, &select_app_callback, &entry, filter, primary.app.c_str(), primary.version.c_str());
...
{
    auto entry = instances_table_entry_t {};

    std::array<const char*, 1> filter = { "id" };
    select_all(apps_table_name, &select_app_callback, &entry, filter, primary.id.c_str());
