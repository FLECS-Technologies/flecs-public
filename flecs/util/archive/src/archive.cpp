...
#include "flecs/util/archive/impl/write_disk.h"
    ...
    -> int
{
    /* write archive header */
    {
        ...
        if (ec) {
            std::fprintf(stderr, "Could not build path relative to %s for %s\n", wd.c_str(), file.c_str());
            return -1;
        }
...
{
    auto ec = std::error_code{};
    for (auto it = fs::recursive_directory_iterator(file, ec); it != fs::recursive_directory_iterator{};
         ++it) {
