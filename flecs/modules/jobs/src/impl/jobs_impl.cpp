...
{
    auto job =
        std::find_if(_job_progress.begin(), _job_progress.end(), [&job_id](const jobs::progress_t& elem) {
            return elem.job_id() == job_id;
        });
    ...
    -> result_t
{
    ...
    const auto it =
        std::find_if(_job_progress.cbegin(), _job_progress.cend(), [&job_id](const jobs::progress_t& elem) {
            return elem.job_id() == job_id;
        });
    ...
    -> void
{
    ...
    while (!g_stop) {
        ...
        auto job_thread = std::thread{[this, job = std::move(job)]() {
            ...
            auto& job_progress =
                *std::find_if(_job_progress.begin(), _job_progress.end(), [this](jobs::progress_t& item) {
