...
impl TryFrom<flecs_app_manifest::AppManifestSingle> for AppManifestSingle {
    type Error = Error;

    fn try_from(value: flecs_app_manifest::AppManifestSingle) -> Result<Self, Self::Error> {
...
pub mod tests {
    ...
    fn try_from_duplicate_environment_err() {
        ...
        let manifest = flecs_app_manifest::AppManifest::try_from(manifest).unwrap();
        assert!(AppManifest::try_from(manifest).is_err())
    }
}
