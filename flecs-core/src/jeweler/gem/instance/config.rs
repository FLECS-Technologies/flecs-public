...
fn extract_if<T, F>(vec: &mut Vec<T>, f: F) -> Option<T>
where
    F: Fn(&T) -> bool,
{
...
impl InstancePortMapping {
    ...
    fn delete_port_mapping_vec(
        ...
        extract_if(existing_port_mappings, |mapping| {
...
pub(crate) mod tests {
    ...
    fn extract_if_some() {
        ...
        assert_eq!(extract_if(&mut vec, |v| *v == 4), Some(4));
    ...
    fn extract_if_none() {
        let mut vec = vec![1, 2, 3, 4, 5, 6];
        assert!(extract_if(&mut vec, |v| *v == 0).is_none());
    }

    #[test]
    fn extract_if_first() {
        let mut vec = vec![(1, 1), (2, 2), (2, 3), (2, 4), (4, 5), (6, 7)];
        assert_eq!(extract_if(&mut vec, |v| v.0 == 2), Some((2, 2)));
