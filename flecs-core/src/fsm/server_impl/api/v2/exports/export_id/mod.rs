...
use flecsd_axum_server::apis::flecsport::ExportsExportIdGetResponse as GetResponse;
use flecsd_axum_server::models::ExportsExportIdGetPathParams as GetPathParams;
        ...
        Ok(Some(mut file)) => {
            ...
            if let Err(e) = file.read_to_end(&mut data).await {
                ...
                GetResponse::Status200_Success {
                    content_length: data.len() as i32,
                    body: types::ByteArray(data),
                }
            }
        }
...
mod tests {
    ...
    async fn get_200() {
        ...
        assert_eq!(
            ...
            .await,
            GetResponse::Status200_Success {
                body: types::ByteArray(TEST_DATA.to_vec()),
                content_length: TEST_DATA.len() as i32,
            }
        );
    }
