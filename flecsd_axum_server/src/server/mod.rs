...
struct InstancesInstanceIdConfigNetworksPostBodyValidator<'a> {
    #[validate(nested)]
    body: &'a models::InstanceConnectToNetworkAdapter,
}
...
fn instances_instance_id_config_networks_post_validation(
    ...
    body: models::InstanceConnectToNetworkAdapter,
) -> std::result::Result<
    (
        models::InstancesInstanceIdConfigNetworksPostPathParams,
        models::InstanceConnectToNetworkAdapter,
    ),
    ValidationErrors,
> {
    path_params.validate()?;
...
async fn instances_instance_id_config_networks_post<I, A>(
    ...
    State(api_impl): State<I>,
    Json(body): Json<models::InstanceConnectToNetworkAdapter>,
) -> Result<Response, StatusCode>
where
    I: AsRef<A> + Send + Sync,
    A: apis::instances::Instances,
{
    ...
    let resp = match result {
                                            Ok(rsp) => match rsp {
                                                apis::instances::InstancesInstanceIdConfigNetworksPostResponse::Status200_InstanceWasAlreadyConnected
                                                => {
                                                  let mut response = response.status(200);
                                                  response.body(Body::empty())
                                                },
                                                ...
                                                apis::instances::InstancesInstanceIdConfigNetworksPostResponse::Status404_InstanceIdOrNetworkAdapterNotFound
                                                => {
                                                  let mut response = response.status(404);
                                                  response.body(Body::empty())
                                                },
