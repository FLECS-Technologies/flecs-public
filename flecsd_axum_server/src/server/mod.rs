...
#[derive(validator::Validate)]
#[allow(dead_code)]
struct InstancesInstanceIdConfigDevicesUsbPortPutBodyValidator<'a> {
    #[validate(nested)]
    body: &'a models::InstancesInstanceIdConfigDevicesUsbPortPutRequest,
}
...
fn instances_instance_id_config_devices_usb_port_put_validation(
    ...
    (
        ...
        Option<models::InstancesInstanceIdConfigDevicesUsbPortPutRequest>,
    ),
    ValidationErrors,
> {
    path_params.validate()?;
    if let Some(body) = &body {
        let b = InstancesInstanceIdConfigDevicesUsbPortPutBodyValidator { body };
        b.validate()?;
    }
...
async fn instances_instance_id_config_devices_usb_port_put<I, A>(
    ...
    State(api_impl): State<I>,
    Json(body): Json<Option<models::InstancesInstanceIdConfigDevicesUsbPortPutRequest>>,
) -> Result<Response, StatusCode>
where
    I: AsRef<A> + Send + Sync,
    A: apis::instances::Instances,
{
    #[allow(clippy::redundant_closure)]
    let validation = tokio::task::spawn_blocking(move || {
        instances_instance_id_config_devices_usb_port_put_validation(path_params, body)
    })
    .await
    .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .instances_instance_id_config_devices_usb_port_put(method, host, cookies, path_params, body)
        .await;
