...
#[doc = "Generated types for `FLECS App Manifest` for tag/branch 2.0.0 - 4ee5f0b996cd2b1773b39d2e17fb46c380ae9086"]
...
///      },
///      "$comment": "https://regex101.com/r/lv1gKP/2"
///    },
///    "hostname": {
...
pub struct FlecsAppManifest {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub editor: Option<FlecsAppManifestEditor>,
...
/// </details>
#[derive(Clone, Debug, Eq, Hash, Ord, PartialEq, PartialOrd, Serialize)]
pub struct FlecsAppManifestEditor(String);
impl std::ops::Deref for FlecsAppManifestEditor {
    type Target = String;
    fn deref(&self) -> &String {
        &self.0
    }
}
impl From<FlecsAppManifestEditor> for String {
    fn from(value: FlecsAppManifestEditor) -> Self {
        value.0
    }
}
impl From<&FlecsAppManifestEditor> for FlecsAppManifestEditor {
    fn from(value: &FlecsAppManifestEditor) -> Self {
        value.clone()
    }
}
impl std::str::FromStr for FlecsAppManifestEditor {
    type Err = self::error::ConversionError;
    fn from_str(value: &str) -> Result<Self, self::error::ConversionError> {
        if regress::Regex::new(
                "^(?::([6][5][5][3][0-5]|[6][5][5][0-2][0-9]|[6][5][0-4][0-9]{2}|[6][0-4][0-9]{3}|[1-5][0-9]{4}|[1-9][0-9]{0,3}))?$",
            )
            .unwrap()
            .find(value)
            .is_none()
        {
            return Err(
                "doesn't match pattern \"^(?::([6][5][5][3][0-5]|[6][5][5][0-2][0-9]|[6][5][0-4][0-9]{2}|[6][0-4][0-9]{3}|[1-5][0-9]{4}|[1-9][0-9]{0,3}))?$\""
                    .into(),
            );
        }
        Ok(Self(value.to_string()))
...
impl std::convert::TryFrom<&str> for FlecsAppManifestEditor {
    type Error = self::error::ConversionError;
    fn try_from(value: &str) -> Result<Self, self::error::ConversionError> {
        value.parse()
    }
}
impl std::convert::TryFrom<&String> for FlecsAppManifestEditor {
    type Error = self::error::ConversionError;
    fn try_from(value: &String) -> Result<Self, self::error::ConversionError> {
        value.parse()
    }
...
impl<'de> serde::Deserialize<'de> for FlecsAppManifestEditor {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
...
impl std::str::FromStr for FlecsAppManifestEnvItem {
    ...
    fn from_str(value: &str) -> Result<Self, self::error::ConversionError> {
        ...
        {
            return Err(
                "doesn't match pattern \"^[a-zA-Z]+(?:[a-zA-Z0-9_\\-\\.])*[=:].*$\""
                    .into(),
            );
        }
        Ok(Self(value.to_string()))
...
pub mod builder {
    ...
    pub struct FlecsAppManifest {
        ...
        editor: Result<Option<super::FlecsAppManifestEditor>, String>,
    ...
    impl FlecsAppManifest {
        ...
        where
            T: std::convert::TryInto<Option<super::FlecsAppManifestEditor>>,
            T::Error: std::fmt::Display,
        {
            self.editor = value
                .try_into()
                .map_err(|e| {
                    format!("error converting supplied value for editor: {}", e)
                });
            self
        }
